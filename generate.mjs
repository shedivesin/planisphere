...
const draw_bottom = true;
const draw_top = true;
const top_color = (draw_bottom && draw_top)? "red": "black";
...
if(draw_bottom) {
  ...
  // Lower plate pivot.
  console.log("<circle cx=\"%d\" cy=\"%d\" r=\"0.5\"/>", w / 2, h / 2);
...
if(draw_top) {
  ...
  // Upper plate pivot.
  console.log("<circle cx=\"%d\" cy=\"%d\" r=\"0.5\" fill=\"%s\"/>", w / 2, h / 2, top_color);
  ...
  for(let min = 0; min < 1440; min += 5) {
    ...
    if(min % 60 === 0) {
      console.log(
        "<text transform=\"translate(%d, %d) rotate(%d) translate(0, 2)\" text-anchor=\"middle\" dominant-baseline=\"hanging\" font-family=\"Helvetica Neue\" font-size=\"9\" font-weight=\"300\" fill=\"%s\">%s</text>",
  ...
  // Upper plate window.
  {
...

    {
      ...
      console.log("<path d=\"%s\" fill=\"none\" stroke=\"%s\"/>", d, top_color);
    }

    {
      ...
      console.log("<path d=\"%s\" fill=\"none\" stroke=\"%s\" stroke-width=\"0.5\"/>", d, top_color);
    }
  }
